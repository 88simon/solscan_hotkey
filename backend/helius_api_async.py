"""
Helius API Integration for Solana Token Analysis (Async Version)
Provides async functions to analyze tokens and extract early bidder data
"""
...
class HeliusAPIAsync:
    """Async wrapper for Helius RPC and Enhanced API endpoints"""
    ...
    async def get_wallet_balance(self, wallet_address: str) -> tuple[Optional[float], int]:
        """
    ...
    async def _rpc_call(self, method: str, params: list) -> dict:
        """Make an async JSON-RPC call to Helius"""
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": method,
    ...
    async def _enhanced_call(self, endpoint: str, params: dict) -> dict:
        """Make an async call to Helius Enhanced API"""
    ...
    async def get_token_metadata(self, mint_address: str) -> tuple[Optional[Dict], int]:
        """
        ...
        try:
            ...
            result = await self._enhanced_call('token-metadata', {
    ...
    async def get_token_creation_time(self, mint_address: str) -> tuple[Optional[int], int]:
        """
    ...
    async def get_parsed_transactions(self, address: str, limit: int = 100, get_earliest: bool = False, token_creation_time: int = None, max_credits: int = 1000) -> tuple[List[Dict], int]:
        """
        ...
        try:
            if get_earliest:
                ...
                return await self._get_earliest_transactions_new(address, limit, token_creation_time, max_credits)
            ...
            signatures = await self._rpc_call('getSignaturesForAddress', [
                address,
                {"limit": limit}
            ])
            ...
            for i, signature in enumerate(sig_list):
                ...
                try:
                    ...
                    tx_data = await self._rpc_call('getTransaction', [
                        signature,
                        {
                            "encoding": "jsonParsed",
    ...
    async def _get_earliest_transactions_new(self, address: str, limit: int = 500, token_creation_time: int = None, max_credits: int = 1000) -> tuple[List[Dict], int]:
        """
        ...
        try:
            ...
            while remaining_limit > 0 and api_calls < max_api_calls:
                ...
                # Make the RPC call
                result = await self._rpc_call('getTransactionsForAddress', params)
            ...
            return await self._get_earliest_transactions_old(address, limit, token_creation_time)

    async def _get_earliest_transactions_old(self, address: str, limit: int = 500, token_creation_time: int = None) -> tuple[List[Dict], int]:
        """
    ...
    async def analyze_token_early_bidders(
        self,
        mint_address: str,
        min_usd: float = 50.0,
        ...
        # Get token metadata
        token_info, metadata_credits = await self.get_token_metadata(mint_address)
        if token_info:
        ...
        token_creation_time, creation_time_credits = await self.get_token_creation_time(mint_address)
        ...
        if token_creation_time:
            transactions, transaction_credits = await self.get_parsed_transactions(
                mint_address,
        ...
        # Fetch wallet balances in parallel for all early bidders
        print(f"[Helius] Fetching wallet balances for {len(early_bidders)} wallets (parallel)...")
        balance_tasks = [
            self.get_wallet_balance(bidder['wallet_address'])
            for bidder in early_bidders
        ]
        balance_results = await asyncio.gather(*balance_tasks)

        # Assign results and sum credits
        balance_credits = 0
        for bidder, (wallet_balance_usd, credits) in zip(early_bidders, balance_results):
            bidder['wallet_balance_usd'] = wallet_balance_usd
            balance_credits += credits

        print(f"[Helius] Wallet balances fetched in parallel (used {balance_credits} credits)")
...
class TokenAnalyzerAsync:
    """High-level async token analysis interface"""

    def __init__(self, api_key: str):
        self.helius = HeliusAPIAsync(api_key)
    ...
    async def analyze_token(
        ...
        """
        return await self.helius.analyze_token_early_bidders(
